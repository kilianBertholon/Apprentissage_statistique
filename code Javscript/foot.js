...
(async () => {
  ...
  let currentPage = 1;
  const maxPages = 350; // ou tout autre nombre maximal de pages que tu souhaites scraper

  await page.waitForTimeout(10000); // Attend un peu pour que la nouvelle page se charge (ajuste si nécessaire)
  while (currentPage <= maxPages) {
    ...
    fs.appendFile('resultat_football_2019.json', JSON.stringify(result, null, 2), (err) => {
      if (err) throw err;
