...
-   Le nombre de but(s) qu'il a inscrit

-   Le nombre de passe(s) décisive(s) qu'il a délivré
...
-   La moyenne de passes clés par match

-   Le pourcentage de centres réussis

-   Le nombre de dégagement par match

-   Le nombre d'interception moyens par match

-   Le nombre de tacle moyen par match

-   Le nombre de Tir bloqué moyen par match

-   Le nombre de dribbles subis moyen par match

-   Le nombre de faute moyen par match

-   Le nombre de tacle moyen recu par match
...
# Division en jeu d'entraînement
set.seed(123)
data_split <- initial_split(all_data, prop = 0.8, strata = Note)
data_train <- training(data_split)
data_test <- testing(data_split)
...
lm_fit <- lm(Note ~ ., data = data_train)

# Prédictions sur l'ensemble de test
predictions_lin <- predict(lm_fit, newdata = data_test)
...
comparison_lin <- data.frame(
  ...
  Residuals = actual_values - predictions_lin
)

# Calcul des statistiques d'évaluation
median_diff <- median(comparison_lin$Actual - comparison_lin$Predicted)
ci_low <- quantile(comparison_lin$Actual - comparison_lin$Predicted, 0.025)
ci_high <- quantile(comparison_lin$Actual - comparison_lin$Predicted, 0.975)
...
ggplot(comparison_lin, aes(x = rownames(comparison_lin), y = Residuals, color = Actual)) +
  ...
  geom_hline(yintercept = ci_low, linetype = "dashed", color = "black", size = 0.5)+ 
  geom_hline(yintercept = ci_high, linetype = "dashed", color = "black", size = 0.5)+ 
  labs(title = "Prédiction Régression Linéaire sur la population",
       x = "Individu",
       y = "Différence") +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  theme_classic() 
...
erreur_moy_abs_tot_reg <- mean(abs(comparison_lin$Actual - comparison_lin$Predicted))
```
...
# Entraînement du modèle de régression linéaire
lm_fit_att <- lm(Note ~ ., data = all_data_poste_train)

# Prédictions sur l'ensemble de test
predictions_poste_lin_att <- predict(lm_fit_att, newdata = all_data_poste_test)
actual_values <- all_data_poste_test$Note  

# Création du dataframe de comparaison
comparison_poste_lin_att <- data.frame(
  Actual = actual_values,
  Predicted = predictions_poste_lin_att,
  Residuals = actual_values - predictions_poste_lin_att
)

# Calcul des statistiques d'évaluation
median_diff <- median(comparison_poste_lin_att$Actual - comparison_poste_lin_att$Predicted)
ci_low <- quantile(comparison_poste_lin_att$Actual - comparison_poste_lin_att$Predicted, 0.025)
ci_high <- quantile(comparison_poste_lin_att$Actual - comparison_poste_lin_att$Predicted, 0.975)
...
ggplot(comparison_poste_lin_att, aes(x = rownames(comparison_poste_lin_att), y = Residuals, color = Actual)) +
  ...
  geom_hline(yintercept = ci_low, linetype = "dashed", color = "black", size = 0.5)+ 
  geom_hline(yintercept = ci_high, linetype = "dashed", color = "black", size = 0.5)+ 
  labs(title = "Prédiction Régression Linéaire sur la population des attaquants",
       x = "Individu",
       y = "Différence") +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  theme_classic() 
...
erreur_moy_abs_att_reg <- mean(abs(comparison_poste_lin_att$Actual - comparison_poste_lin_att$Predicted))
```
...
set.seed(123)
data_split <- initial_split(all_data, prop = 0.8, strata = Note)
data_train_2 <- training(data_split)
data_test_2 <- testing(data_split)

# Préparation des données
data_train_3 <- subset(data_train_2, select = -c(nom, Poste, Club, Annee, Poste_global))
data_train <- na.omit(data_train_3, cols = "Note")

data_test_3 <- subset(data_test_2, select = -c(nom, Poste, Club, Annee, Poste_global))
data_test <- na.omit(data_test_3, cols = "Note")
...
# Création du workflow pour le LASSO
lasso_wf <- workflow() |> 
  add_model(lasso_model) |> 
  add_formula(Note ~ .)

# Création des folds pour la validation croisée
folds <- vfold_cv(data_train, v = 3, strata = "Note")

# Définition de la grille de recherche du paramètre lambda
lambda_grid <- grid_regular(penalty(range = c(-5, 2)), levels = 100)

# Validation croisée pour le LASSO
lasso_cv <- lasso_wf |> 
  tune_grid(resamples = folds, grid = lambda_grid, metrics = metric_set(rsq))

# Collecte des métriques
res <- lasso_cv |> collect_metrics()
...
best_rsq <- lasso_cv |> select_best(metric = "rsq")
best_rsq$penalty

# Finalisation du workflow avec le meilleur lambda
final_wf <- finalize_workflow(lasso_wf, best_rsq)
print(final_wf)

# Entraînement du modèle LASSO avec le meilleur lambda sur l'ensemble des données d'entraînement
final_lasso <- final_wf |> fit(data_train)

# Tidy sur le modèle LASSO
final_lasso |> tidy()

# Prédictions avec le modèle LASSO sur l'ensemble de test
predictions_lasso <- predict(final_lasso, new_data = data_test)
comparison_lasso <- data.frame(Actual = data_test$Note, Predicted = predictions_lasso)
...
# Calcul des différences et des erreurs absolues
...
# Calcul de la médiane et des intervalles de confiance
...
ggplot(comparison_lasso, aes(x = rownames(comparison_lasso), y = Residuals, color = Actual)) +
  ...
  geom_hline(yintercept = ci_low, linetype = "dashed", color = "black", size = 0.5)+ 
  geom_hline(yintercept = ci_high, linetype = "dashed", color = "black", size = 0.5)+ 
  labs(title = "Prédiction Régression LASSO sur la population",
       x = "Individu",
       y = "Différence") +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  theme_classic() 
...
comparison_poste_lasso_att <- data.frame(Actual = all_data_poste_test$Note, Predicted = predictions_poste_lasso_att)
...
## Afin d'éviter de relancer tout les modèles nous placons ici un récap des différent smodèles
recap <- data.frame(
  Model = c("XG_att", "XG_mil", "XG_def", "XG_gar", "XG_tot", 
            "Reg_tot", "Reg_att", "Reg_LASSO_tot", "Reg_LASSO_att", 
            "Reg_RIDGE_tot", "Reg_RIDGE_att", "Rf_tot"),
  R2 = c(0.4680, 0.4536, 0.3587, 0.0549, 0.3894, 0.3304, 
         0.5126, 0.3304, 0.5130, 0.3256, 0.5118, 0.3882),
  RMSE = c(0.2650, 0.2600, 0.2543, 0.3741, 0.2837, 0.2922, 
           0.2311, 0.3788, 0.2310, 0.3734, 0.2312, 0.2793),
  DifMoyAbs = c(0.1386, 0.1431, 0.1449, 0.2524, 0.1517, 0.1643, 
                0.1285, 0.1284, 0.1641, 0.1622, 0.1283, 0.1541)
)

library(ggplot2)
library(patchwork)

# Filtrer les modèles contenant "tot" dans leur nom
recap_tot <- recap[grep("tot", recap$Model), ]

# Créer trois graphiques linéaires pour chaque variable
# R2
plot_r2 <- ggplot(recap_tot, aes(x = Model, y = R2, group = 1)) +
  geom_line() +
  labs(
       x = "Modèle",
       y = "R2") +
...
# RMSE
plot_rmse <- ggplot(recap_tot, aes(x = Model, y = RMSE, group = 1)) +
  geom_line() +
  labs(
       x = "Modèle",
       y = "RMSE") +
...
# DifMoyAbs
plot_dif <- ggplot(recap_tot, aes(x = Model, y = DifMoyAbs, group = 1)) +
  geom_line() +
  labs(
       x = "Modèle",
       y = "DifMoyAbs") +
...
# Combinaison des trois graphiques
combined_plot <- plot_r2 + plot_rmse + plot_dif
...
On peut donc ici voir que XGboost produit les meilleurs résultats sur la population globale
...
all_data_xg <- subset(test_data, select = -c(nom, Poste, Club, Annee, Poste_global, minJ, But, PasseDecisive))
all_data_xg <- na.omit(all_data_xg, cols = "Note")
...
gg_plot
R2_tot_xg_trans1 <- 1 - sum((data_test_fin_2$Note - data_test_fin_2$.pred)^2) / sum((data_test_fin_2$Note - mean(data_test_fin_2$Note))^2)
RMSE_tot_xg_trans1 <- sqrt(mean((data_test_fin_2$Note - data_test_fin_2$.pred)^2))
erreur_moy_abs_tot_xg_trans1 <- mean(data_test_fin_2$DifferenceAbs)


```

-   Si on se fixe également sur un point de vue sportif. Evaluer des joueurs qui joue très peu dans un match ou sur l'ensemble de la saison ne nous semble pas cohérent (nous décidons donc de laisser uniquement les joueurs jouant plus de 270 min sur la saison et plus de 45 min par apparitions
...
all_data_xg <- subset(all_data_2, select = -c(nom, Poste, Club, Annee, Poste_global, minJ, But, PasseDecisive))
all_data_xg <- na.omit(all_data_xg, cols = "Note")
...
gg_plot
R2_tot_xg_trans2 <- 1 - sum((data_test_fin_2$Note - data_test_fin_2$.pred)^2) / sum((data_test_fin_2$Note - mean(data_test_fin_2$Note))^2)
RMSE_tot_xg_trans2 <- sqrt(mean((data_test_fin_2$Note - data_test_fin_2$.pred)^2))
erreur_moy_abs_tot_xg_trans2 <- mean(data_test_fin_2$DifferenceAbs)
```

-   Enfin, on voit que la grande majorité des joueurs ont des notes compris entre 6 et 8 et on voit sur les graphes associés à chaque modèles que l'on a beaucoup de mal à prédire la performance de joueurs ayant des notes un peu atypiques. On décide donc de conserver uniquement les joeueurs avec une notes comprises entre 6 et 8
...
all_data_2 <- test_data |> filter(minJ >270)
all_data_2 <- all_data_2 |> filter(MJ_match > 45)
all_data_3 <- all_data_2 |> filter(Note >= 6)
all_data_4 <- all_data_3 |> filter(Note <= 8)
...
gg_plot
R2_tot_xg_trans3 <- 1 - sum((data_test_fin_2$Note - data_test_fin_2$.pred)^2) / sum((data_test_fin_2$Note - mean(data_test_fin_2$Note))^2)
RMSE_tot_xg_trans3 <- sqrt(mean((data_test_fin_2$Note - data_test_fin_2$.pred)^2))
erreur_moy_abs_tot_xg_trans3 <- mean(data_test_fin_2$DifferenceAbs)
```

```{r}
#Amélioration de modèle 
df_recap_xg <- data.frame(
  Model = c("Xg_tot", "Xg_tot_par_match", "Xg_tot_par_match_tpsJeu", "Xg_tot_par_match_tpsJeu_Note6_8"),
  R2 = c(R2_tot_xg, R2_tot_xg_trans1, R2_tot_xg_trans2, R2_tot_xg_trans3),
  RMSE = c(RMSE_tot_xg, RMSE_tot_xg_trans1, RMSE_tot_xg_trans2, RMSE_tot_xg_trans3),
  DiffAbsMoy = c(erreur_moy_abs_tot_xg, erreur_moy_abs_tot_xg_trans1, erreur_moy_abs_tot_xg_trans2, erreur_moy_abs_tot_xg_trans3)
)

plot_r2 <- ggplot(df_recap_xg, aes(x = Model, y = R2, group = 1)) +
  geom_line() +
  labs(
       x = "Modèle",
       y = "R2") +
...
# RMSE
plot_rmse <- ggplot(df_recap_xg, aes(x = Model, y = RMSE, group = 1)) +
  geom_line() +
  labs(
       x = "Modèle",
       y = "RMSE") +
...
# DifMoyAbs
plot_dif <- ggplot(df_recap_xg, aes(x = Model, y = DiffAbsMoy, group = 1)) +
  geom_line() +
  labs(
       x = "Modèle",
       y = "DifMoyAbs") +
...
Pour conclure, le fil du rouge de ce projet à été d'utiliser les différents modèles de Machine learning vue en cours dans le but de répondre à une problématique (dans notre cas un problème de régression). En se basant sur 3 indicateurs de performances des modèles de régression (R2, RMSE et et diffAbsMoy, nous avons essayer d'estimer au mieux les notes de joueurs à partir d'un ensemble de variables). L'ensemble des modèles à été FineTuner par validation croisée pour sélectionner les meilleurs paramètres pour chacun des modèles.

Enfin, après sélection du modèle le plus adapté (ici XGboost). Nous avons essayé d'améliorer la prédiction de notre modèle en ajoutant des éléments restrictifs sur nos données (dans la mesure du raisonnables bien sûr).
